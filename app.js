...
function initSync() {
    ...
    // Custom premium setups
    setupThemeToggleButton();
    setupPresetAngles();
    setupCanvasDoubleClicks();
    setupBadgePicker();
    setupAuroraMeshPresets();
    setupTemplatesModal();
...
function syncUIWithState() {
    ...
    document.getElementById('aurora-options').style.display = bg.type === 'aurora' ? 'block' : 'none';
    
    // Aurora Mesh Preset select sync
    if (bg.type === 'aurora') {
        const activePreset = bg.auroraPreset || 'peach';
        document.querySelectorAll('#aurora-presets .preset-swatch').forEach(s => {
            s.style.borderColor = s.dataset.aurora === activePreset ? 'var(--accent)' : 'transparent';
        });
    }
...
function setupEventListeners() {
    ...
    document.querySelectorAll('#bg-type-selector button').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById('aurora-options').style.display = btn.dataset.type === 'aurora' ? 'block' : 'none';
...
function drawBackgroundToContext(context, dims, bg) {
    if (bg.type === 'gradient') {
        ...
        const preset = bg.auroraPreset || 'peach';
        let baseColor = '#a1c4fd';
        let blobs = [];

        if (preset === 'peach') {
            baseColor = '#a1c4fd';
            blobs = [
                { x: 0.2, y: 0.3, r: 0.6, color: '#ff9a9e' },
                { x: 0.8, y: 0.7, r: 0.6, color: '#fecfef' }
            ];
        } else if (preset === 'aurora') {
            baseColor = '#0f172a';
            blobs = [
                { x: 0.3, y: 0.2, r: 0.7, color: '#059669' },
                { x: 0.7, y: 0.8, r: 0.7, color: '#0891b2' }
            ];
        } else if (preset === 'sunset') {
            baseColor = '#1e1b4b';
            blobs = [
                { x: 0.2, y: 0.8, r: 0.7, color: '#db2777' },
                { x: 0.8, y: 0.2, r: 0.7, color: '#ea580c' }
            ];
        } else if (preset === 'frost') {
            baseColor = '#4b5563';
            blobs = [
                { x: 0.1, y: 0.3, r: 0.6, color: '#4facfe' },
                { x: 0.9, y: 0.9, r: 0.6, color: '#00f2fe' }
            ];
        } else if (preset === 'velvet') {
            baseColor = '#030712';
            blobs = [
                { x: 0.5, y: 0.2, r: 0.7, color: '#7c3aed' },
                { x: 0.2, y: 0.8, r: 0.6, color: '#db2777' }
            ];
        } else if (preset === 'sage') {
            baseColor = '#f1f5f9';
            blobs = [
                { x: 0.2, y: 0.2, r: 0.6, color: '#a7f3d0' },
                { x: 0.8, y: 0.8, r: 0.6, color: '#fed7aa' }
            ];
        }
        ...
        blobs.forEach(blob => {
            const bx = blob.x * dims.width;
            const by = blob.y * dims.height;
            const br = blob.r * dims.width;

            const radGrad = context.createRadialGradient(bx, by, 0, bx, by, br);
...
// ==========================================
// CUSTOM PREMIUM ADDITIONS (AURAMOCK 3D)
// ==========================================
...
function setupThemeToggleButton() {
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        // Sync setting buttons if any exist
        document.querySelectorAll('#theme-selector button').forEach(btn => {
...
function setupPresetAngles() {
    document.querySelectorAll('.angle-preset-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const angleType = btn.dataset.angle;
            const ss = getScreenshotSettings();
            ...
            switch(angleType) {
                case 'front':
                    ...
                    break;
                case 'iso-left':
                    rotX = 20; rotY = -25; rotZ = 10;
                    break;
                case 'iso-right':
                    rotX = 20; rotY = 25; rotZ = -10;
                    break;
                case 'subtle-left':
                    rotX = 5; rotY = -12; rotZ = 2;
                    break;
                case 'subtle-right':
                    rotX = 5; rotY = 12; rotZ = -2;
                    break;
                case 'pitch':
                    rotX = 30; rotY = 0; rotZ = -5;
                    break;
            }

            // Update state
            ss.rotation3D.x = rotX;
            ss.rotation3D.y = rotY;
            ss.rotation3D.z = rotZ;

            // Sync UI sliders
            const sliderX = document.getElementById('rotation-3d-x');
            const sliderY = document.getElementById('rotation-3d-y');
            const sliderZ = document.getElementById('rotation-3d-z');
            if (sliderX) {
                sliderX.value = rotX;
                document.getElementById('rotation-3d-x-value').textContent = rotX + '°';
            }
            if (sliderY) {
                sliderY.value = rotY;
                document.getElementById('rotation-3d-y-value').textContent = rotY + '°';
            }
            if (sliderZ) {
                sliderZ.value = rotZ;
                document.getElementById('rotation-3d-z-value').textContent = rotZ + '°';
            }

            // Rotate in Three.js and redraw
            if (typeof setThreeJSRotation === 'function') {
                setThreeJSRotation(rotX, rotY, rotZ);
            }
            updateCanvas();
        });
    });
...
function setupCanvasDoubleClicks() {
    ...
    const editor = document.getElementById('floating-text-editor');
    const headlineInput = document.getElementById('floating-headline-input');
    const subheadlineInput = document.getElementById('floating-subheadline-input');
    const cancelBtn = document.getElementById('floating-text-cancel');
    const saveBtn = document.getElementById('floating-text-save');
    ...
    function handleDblClick(e) {
        ...
        headlineInput.value = (txt.headlines && txt.headlines[headlineLang]) || '';
        subheadlineInput.value = (txt.subheadlines && txt.subheadlines[subheadlineLang]) || '';

        // Position editor near click but keep on screen
        editor.style.display = 'block';
        const editorWidth = 340;
        const editorHeight = 220;
        let left = e.clientX - editorWidth / 2;
        let top = e.clientY - editorHeight - 15;

        // Clamping inside window
        left = Math.max(10, Math.min(window.innerWidth - editorWidth - 10, left));
        top = Math.max(10, Math.min(window.innerHeight - editorHeight - 10, top));
    ...
    if (threejsContainer) {
        threejsContainer.addEventListener('dblclick', handleDblClick);
    }
    ...
    function saveChanges() {
        ...
        if (!txt.headlines) txt.headlines = {};
        if (!txt.subheadlines) txt.subheadlines = {};

        txt.headlines[headlineLang] = headlineInput.value;
        txt.subheadlines[subheadlineLang] = subheadlineInput.value;

        // Enable if edited and was disabled
        if (headlineInput.value.trim() !== '') {
            txt.headlineEnabled = true;
        }
        if (subheadlineInput.value.trim() !== '') {
            txt.subheadlineEnabled = true;
        }

        // Sync to sidebar UI textareas
        const sidebarHeadline = document.getElementById('headline-text');
        const sidebarSubheadline = document.getElementById('subheadline-text');
        if (sidebarHeadline) sidebarHeadline.value = headlineInput.value;
        if (sidebarSubheadline) sidebarSubheadline.value = subheadlineInput.value;
    ...
    subheadlineInput.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') editor.style.display = 'none';
        if (e.key === 'Enter' && e.ctrlKey) saveChanges();
    });
    ...
    document.addEventListener('mousedown', (e) => {
        if (editor.style.display === 'block' && !editor.contains(e.target) && e.target !== canvas && e.target !== threejsContainer) {
            editor.style.display = 'none';
        }
    });
}

const appStoreBadgeSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 135 40" width="135" height="40">
  <rect width="135" height="40" rx="6" fill="#000000" stroke="#a6a6a6" stroke-width="0.8"/>
  <path d="M22.5 15.2c.1-2.9 2.4-4.3 2.5-4.4-1.4-2-3.5-2.2-4.2-2.3-1.8-.2-3.6 1-4.5 1-.9 0-2.4-.9-3.9-.9-2-.1-3.9 1.1-4.9 2.9-2.1 3.6-.5 9-2.5 12.5-1 1.7-2.2 3.6-4 3.5-1.7-.1-2.4-1.1-4.3-1.1-1.9 0-2.6 1.1-4.2 1.1-1.6 0-2.8-1.7-3.8-3.5-2.3-3.3-4-9.4-1.6-13.6 1.2-2 3.2-3.3 5.4-3.3 1.7 0 3.3 1.2 4.3 1.2 1 0 3-.1 5-.1m-4.3-8c.9-1.1 1.5-2.6 1.3-4.1-1.3.1-2.9.9-3.8 2-0.8.9-1.5 2.5-1.3 4 1.5.1 3-0.7 3.8-1.9z" fill="#ffffff" transform="scale(0.85) translate(8, 7)"/>
  <text x="45" y="15" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="8" font-weight="500">Download on the</text>
  <text x="45" y="29" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="14" font-weight="600">App Store</text>
</svg>`;

const googlePlayBadgeSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 135 40" width="135" height="40">
  <rect width="135" height="40" rx="6" fill="#000000" stroke="#a6a6a6" stroke-width="0.8"/>
  <g transform="translate(10, 8) scale(0.9)">
    <path d="M3.2 1.4C2.5 2.1 2 3.2 2 4.7v18.7c0 1.5.5 2.6 1.2 3.3L15.3 14.6 3.2 1.4z" fill="#00F0FF"/>
    <path d="M21.2 8.7L15.3 14.6l5.9 5.9 7.1-4c2.0-1.1 2.0-3 0-4.1l-7.1-3.7z" fill="#FFC900"/>
    <path d="M15.3 14.6L3.2 26.7c.6.6 1.6.7 2.7.1l15.3-8.7-5.9-3.5z" fill="#FF1844"/>
    <path d="M15.3 14.6L21.2 8.7 5.9.1C4.8-.5 3.8-.4 3.2.2l12.1 14.4z" fill="#00E676"/>
  </g>
  <text x="42" y="15" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="8" font-weight="500" letter-spacing="0.3">GET IT ON</text>
  <text x="42" y="29" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="13" font-weight="600">Google Play</text>
</svg>`;

const starsBadgeSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 135 30" width="135" height="30">
  <g fill="#fbbf24" stroke="#fbbf24" stroke-width="0.5">
    <polygon points="10 2 13 8 20 9 15 14 16 21 10 18 4 21 5 14 0 9 7 8" transform="translate(5, 3)"/>
    <polygon points="10 2 13 8 20 9 15 14 16 21 10 18 4 21 5 14 0 9 7 8" transform="translate(29, 3)"/>
    <polygon points="10 2 13 8 20 9 15 14 16 21 10 18 4 21 5 14 0 9 7 8" transform="translate(53, 3)"/>
    <polygon points="10 2 13 8 20 9 15 14 16 21 10 18 4 21 5 14 0 9 7 8" transform="translate(77, 3)"/>
    <polygon points="10 2 13 8 20 9 15 14 16 21 10 18 4 21 5 14 0 9 7 8" transform="translate(101, 3)"/>
  </g>
</svg>`;

const laurelBadgeSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 150 70" width="150" height="70">
  <path d="M25 45c0-10 8-18 18-18" fill="none" stroke="#fbbf24" stroke-width="1.5"/>
  <path d="M125 45c0-10-8-18-18-18" fill="none" stroke="#fbbf24" stroke-width="1.5"/>
  <path d="M24 45c0-4 2-8 5-10 1-1 3 0 2 2-2 2-4 5-4 8 0 1-1 1-3 0zm5-10c2-3 5-6 8-8 1-1 2 1 1 2-2 2-5 5-7 8-1 1-1 0-2-2zm7-7c3-2 6-4 10-5 1 0 1 2 0 3-3 1-6 3-9 5 0 0-1 0-1-3z" fill="#fbbf24"/>
  <path d="M126 45c0-4-2-8-5-10-1-1-3 0-2 2 2 2 4 5 4 8 0 1 1 1 3 0zm-5-10c-2-3-5-6-8-8-1-1-2 1-1 2 2 2 5 5 7 8 1 1 1 0 2-2zm-7-7c-3-2-6-4-10-5-1 0-1 2 0 3 3 1 6 3 9 5 0 0 1 0 1-3z" fill="#fbbf24"/>
  <text x="75" y="24" fill="#fbbf24" font-family="-apple-system, sans-serif" font-size="9" font-weight="700" text-anchor="middle" letter-spacing="1">APP OF THE DAY</text>
  <text x="75" y="42" fill="#ffffff" font-family="-apple-system, sans-serif" font-size="12" font-weight="600" text-anchor="middle">EDITORS' CHOICE</text>
  <text x="75" y="56" fill="#fbbf24" font-family="-apple-system, sans-serif" font-size="8" font-weight="500" text-anchor="middle" opacity="0.8">★ ★ ★ ★ ★</text>
</svg>`;

function generateBadgeDataURL(type) {
    let svgText = '';
    if (type === 'appstore') svgText = appStoreBadgeSVG;
    else if (type === 'googleplay') svgText = googlePlayBadgeSVG;
    else if (type === 'stars') svgText = starsBadgeSVG;
    else if (type === 'laurel-appday') svgText = laurelBadgeSVG;
    
    return 'data:image/svg+xml;base64,' + btoa(unescape(encodeURIComponent(svgText)));
}

function addBadgeElement(type) {
    const src = generateBadgeDataURL(type);
    const img = new Image();
    img.onload = () => {
        addGraphicElement(img, src, type + '_badge');
    };
    img.src = src;
}

function setupBadgePicker() {
    const addBadgeBtn = document.getElementById('add-badge-btn');
    const picker = document.getElementById('badge-picker');
    if (!addBadgeBtn || !picker) return;

    addBadgeBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        
        // Hide other pickers if open
        const emojiPicker = document.getElementById('emoji-picker');
        const iconPicker = document.getElementById('icon-picker');
        if (emojiPicker) emojiPicker.style.display = 'none';
        if (iconPicker) iconPicker.style.display = 'none';
    ...
    picker.querySelectorAll('.badge-picker-item').forEach(item => {
        item.addEventListener('click', () => {
            const badgeType = item.dataset.badge;
            addBadgeElement(badgeType);
            picker.style.display = 'none';
        });
    });

    document.addEventListener('mousedown', (e) => {
        if (picker.style.display === 'block' && !picker.contains(e.target) && e.target !== addBadgeBtn) {
            picker.style.display = 'none';
        }
    });
}

function setupAuroraMeshPresets() {
    document.querySelectorAll('#aurora-presets .preset-swatch').forEach(swatch => {
        swatch.addEventListener('click', () => {
            const presetName = swatch.dataset.aurora;
            
            // Update state
            setBackground('type', 'aurora');
            setBackground('auroraPreset', presetName);
...
const exampleTemplates = [
    ...
    },
    {
        ...
        previewGradient: 'radial-gradient(circle at 20% 30%, #ff9a9e 0%, transparent 60%), radial-gradient(circle at 80% 70%, #fecfef 0%, transparent 60%), #a1c4fd',
        bg: { type: 'aurora', auroraPreset: 'peach' },
        screenshot: { use3D: true, device3D: 'iphone', scale: 72, y: 56, x: 50, rotation3D: { x: 20, y: -25, z: 10 } },
        text: { headlineEnabled: true, headlines: { en: 'Vibe to the Cosmic Sounds' }, headlineFont: "'Outfit', sans-serif", headlineSize: 85, headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Stream millions of lo-fi tracks in spatial audio.' }, subheadlineFont: "'Outfit', sans-serif", subheadlineSize: 42, subheadlineColor: '#ffffff', subheadlineOpacity: 85 },
        badges: ['googleplay', 'stars']
    },
    {
        id: 'cybersunset',
        title: 'Cyber Sunset (Gaming)',
        description: 'Intense Cyber Sunset background, rotated 3D Samsung Galaxy S25, bold Cabinet Grotesk font, and rating medals.',
        previewGradient: 'radial-gradient(circle at 20% 80%, #db2777 0%, transparent 60%), radial-gradient(circle at 80% 20%, #ea580c 0%, transparent 60%), #1e1b4b',
        bg: { type: 'aurora', auroraPreset: 'sunset' },
        screenshot: { use3D: true, device3D: 'samsung', scale: 75, y: 55, x: 50, rotation3D: { x: 20, y: 25, z: -10 } },
        text: { headlineEnabled: true, headlines: { en: 'CHALLENGE THE FUTURE' }, headlineFont: "'Cabinet Grotesk', sans-serif", headlineSize: 90, headlineWeight: '800', headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Strap in for a neo-tokyo tactical ride.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#ff007f', subheadlineOpacity: 95 },
        badges: ['appstore', 'laurel-appday']
    },
    {
        id: 'northerndreams',
        title: 'Northern Dreams (Travel)',
        description: 'Serene Northern Lights backdrop, flat front-facing iPhone 15 Pro, elegant serif Playfair Display typography, and rating details.',
        previewGradient: 'radial-gradient(circle at 30% 20%, #059669 0%, transparent 60%), radial-gradient(circle at 70% 80%, #0891b2 0%, transparent 60%), #0f172a',
        bg: { type: 'aurora', auroraPreset: 'aurora' },
        screenshot: { use3D: true, device3D: 'iphone', scale: 70, y: 58, x: 50, rotation3D: { x: 0, y: 0, z: 0 } },
        text: { headlineEnabled: true, headlines: { en: 'Escape to the Wilderness' }, headlineFont: "'Playfair Display', serif", headlineItalic: true, headlineSize: 80, headlineColor: '#e8f5e9', subheadlineEnabled: true, subheadlines: { en: 'Curated Nordic getaways for the conscious explorer.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#a1a1a6', subheadlineOpacity: 80 },
        badges: ['appstore', 'stars']
    },
    {
        id: 'premiumdark',
        title: 'Premium Dark (Finance)',
        description: 'Sleek dark gradient background, high-floating iPhone pitch, and clean typography. Excellent for banking and high-end services.',
        previewGradient: 'linear-gradient(160deg, #0a0a0f 0%, #1a1033 50%, #0d1b2a 100%)',
        bg: { type: 'gradient', gradient: { angle: 160, stops: [{ color: '#0a0a0f', position: 0 }, { color: '#1a1033', position: 50 }, { color: '#0d1b2a', position: 100 }] } },
        screenshot: { use3D: true, device3D: 'iphone', scale: 70, y: 62, x: 50, rotation3D: { x: 30, y: 0, z: -5 } },
        text: { headlineEnabled: true, headlines: { en: 'Wealth management. Simplified.' }, headlineFont: "'Inter', sans-serif", headlineSize: 80, headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'High-yield savings & global index funds.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 42, subheadlineColor: '#a1a1a6', subheadlineOpacity: 70 },
        badges: ['appstore', 'googleplay']
    },
    {
        id: 'sagebreeze',
        title: 'Sage Breeze (Wellness)',
        description: 'Calm Sage Breeze mint-peach aurora background, slightly turned iPhone, and App of the Day laurels.',
        previewGradient: 'radial-gradient(circle at 20% 20%, #a7f3d0 0%, transparent 60%), radial-gradient(circle at 80% 80%, #fed7aa 0%, transparent 60%), #f1f5f9',
        bg: { type: 'aurora', auroraPreset: 'sage' },
        screenshot: { use3D: true, device3D: 'iphone', scale: 72, y: 55, x: 50, rotation3D: { x: 5, y: -12, z: 2 } },
        text: { headlineEnabled: true, headlines: { en: 'Breathe In. Let Go.' }, headlineFont: "'Outfit', sans-serif", headlineSize: 85, headlineColor: '#2c3e50', subheadlineEnabled: true, subheadlines: { en: 'Daily mindfulness sessions & sleep analysis.' }, subheadlineFont: "'Outfit', sans-serif", subheadlineSize: 40, subheadlineColor: '#4f5d73', subheadlineOpacity: 80 },
        badges: ['laurel-appday']
    },
    {
        id: 'emberhorizon',
        title: 'Ember Horizon (Fitness)',
        description: 'Vibrant gym/sport themed orange sunset gradient background, tilted Samsung device, and 5-star ratings.',
        previewGradient: 'linear-gradient(130deg, #f12711 0%, #f5af19 100%)',
        bg: { type: 'gradient', gradient: { angle: 130, stops: [{ color: '#f12711', position: 0 }, { color: '#f5af19', position: 100 }] } },
        screenshot: { use3D: true, device3D: 'samsung', scale: 72, y: 55, x: 50, rotation3D: { x: 5, y: 12, z: -2 } },
        text: { headlineEnabled: true, headlines: { en: 'PUSH YOUR LIMITS' }, headlineFont: "'Syne', sans-serif", headlineSize: 90, headlineWeight: '700', headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Interactive strength training & AI personal coach.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#f5af19', subheadlineOpacity: 90 },
        badges: ['stars']
    },
    {
        id: 'retrovibe',
        title: 'Retro Vibe (Photo Editor)',
        description: 'Trendy Holographic pink-purple background, rotated 3D iPhone, and elegant camera theme styling.',
        previewGradient: 'linear-gradient(135deg, #ff007f 0%, #7f00ff 100%)',
        bg: { type: 'gradient', gradient: { angle: 135, stops: [{ color: '#ff007f', position: 0 }, { color: '#7f00ff', position: 100 }] } },
        screenshot: { use3D: true, device3D: 'iphone', scale: 72, y: 55, x: 50, rotation3D: { x: 20, y: -25, z: 10 } },
        text: { headlineEnabled: true, headlines: { en: 'Shoot Analog. Edit Digitally.' }, headlineFont: "'Playfair Display', serif", headlineSize: 85, headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Over 100+ nostalgic film presets and grain tools.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#ffffff', subheadlineOpacity: 80 },
        badges: ['appstore']
    },
    {
        id: 'minimalwhite',
        title: 'Minimal White (Productivity)',
        description: 'Clean off-white solid background, front-facing dark iPhone, dark Inter typography, and Android download badge.',
        previewGradient: '#f3f4f6',
        bg: { type: 'solid', solid: '#f3f4f6' },
        screenshot: { use3D: true, device3D: 'iphone', scale: 70, y: 58, x: 50, rotation3D: { x: 0, y: 0, z: 0 } },
        text: { headlineEnabled: true, headlines: { en: 'Organize your workday.' }, headlineFont: "'Inter', sans-serif", headlineSize: 80, headlineColor: '#111827', subheadlineEnabled: true, subheadlines: { en: 'Clean checklists, keyboard shortcuts, and calendar sync.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#4b5563', subheadlineOpacity: 90 },
        badges: ['googleplay']
    },
    {
        id: 'foodieexpress',
        title: 'Foodie Express (Delivery)',
        description: 'Warm sherbet background, floating Samsung mockup, Outfit typography, and star rating stickers.',
        previewGradient: 'linear-gradient(135deg, #ff9966 0%, #ff5e62 100%)',
        bg: { type: 'gradient', gradient: { angle: 135, stops: [{ color: '#ff9966', position: 0 }, { color: '#ff5e62', position: 100 }] } },
        screenshot: { use3D: true, device3D: 'samsung', scale: 70, y: 62, x: 50, rotation3D: { x: 30, y: 0, z: -5 } },
        text: { headlineEnabled: true, headlines: { en: 'Hot Meals, Delivered Fast.' }, headlineFont: "'Outfit', sans-serif", headlineSize: 80, headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Track your favorite local diners directly to your door.' }, subheadlineFont: "'Outfit', sans-serif", subheadlineSize: 40, subheadlineColor: '#ffffff', subheadlineOpacity: 85 },
        badges: ['stars']
    },
    {
        id: 'creativefolio',
        title: 'Creative Folio (Portfolio)',
        description: 'Dreamy Velvet Bloom purple-pink aurora background, rotated 3D iPhone, and Editors Choice laurels.',
        previewGradient: 'radial-gradient(circle at 50% 20%, #7c3aed 0%, transparent 60%), radial-gradient(circle at 20% 80%, #db2777 0%, transparent 60%), #030712',
        bg: { type: 'aurora', auroraPreset: 'velvet' },
        screenshot: { use3D: true, device3D: 'iphone', scale: 72, y: 55, x: 50, rotation3D: { x: 20, y: 25, z: -10 } },
        text: { headlineEnabled: true, headlines: { en: 'Showcase Your Work' }, headlineFont: "'Cabinet Grotesk', sans-serif", headlineSize: 85, headlineColor: '#ffffff', subheadlineEnabled: true, subheadlines: { en: 'Beautiful portfolios for painters, photographers, and designers.' }, subheadlineFont: "'Inter', sans-serif", subheadlineSize: 40, subheadlineColor: '#ffffff', subheadlineOpacity: 85 },
        badges: ['laurel-appday']
    }
];
...
function setupTemplatesModal() {
    ...
    const tabContents = document.querySelectorAll('.templates-tab-content');
    const gridContainer = document.getElementById('templates-grid-container');

    if (!templatesBtn || !modal || !gridContainer) return;
    ...
    const bannerBtn = document.getElementById('templates-banner-btn');
    if (bannerBtn) {
    ...
    tabButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            tabContents.forEach(content => {
                content.style.display = content.id === 'templates-tab-' + targetTab ? 'block' : 'none';
            });
        });
    });

    // Render Templates Grid Cards
    // Render Templates Grid Cards
    function renderTemplatesGrid() {
        gridContainer.innerHTML = exampleTemplates.map(tpl => {
            // Read 3D rotation angles for phone styling
            const rx = tpl.screenshot.rotation3D?.x || 0;
            const ry = tpl.screenshot.rotation3D?.y || 0;
            const rz = tpl.screenshot.rotation3D?.z || 0;

            // Generate an abstract representation of the text layout
            const titleText = tpl.text.headlines?.en || 'App Title';

            return `
            <div class="template-card-wrapper" style="perspective: 1000px;">
                <div class="template-card" style="background: var(--bg-primary); border: 1px solid var(--border-color); border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; transition: transform 0.1s ease, box-shadow 0.1s ease; box-shadow: 0 4px 6px rgba(0,0,0,0.1); height: 100%; transform-style: preserve-3d;">
                    <div style="height: 160px; background: ${tpl.previewGradient}; position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; border-bottom: 1px solid var(--border-color); perspective: 800px; transform-style: preserve-3d;">
                        
                        <!-- Floating Background Elements (Stars/Blobs) for depth -->
                        <div style="position: absolute; width: 150%; height: 150%; background: radial-gradient(circle at center, rgba(255,255,255,0.1) 0%, transparent 70%); transform: translateZ(-50px);"></div>
                        
                        <!-- Sleek Glassy Mini Phone Preview Rotated in 3D -->
                        <div class="template-mini-phone" style="width: 54px; height: 110px; background: rgba(255, 255, 255, 0.1); border: 1.5px solid rgba(255,255,255,0.4); border-radius: 8px; box-shadow: 0 15px 35px rgba(0,0,0,0.3), inset 0 0 10px rgba(255,255,255,0.2); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); display: flex; flex-direction: column; align-items: center; position: relative; padding: 3px; box-sizing: border-box; transform-style: preserve-3d; transform: translateZ(30px) rotateX(${rx}deg) rotateY(${ry}deg) rotateZ(${rz}deg); transition: transform 0.5s cubic-bezier(0.16, 1, 0.3, 1);">
                            <!-- Notch/Dynamic Island -->
                            <div style="width: 16px; height: 4px; background: rgba(0,0,0,0.8); border-radius: 3px; margin-top: 2px; margin-bottom: 4px; z-index: 2;"></div>
                            
                            <!-- Inner Screen Graphic (App Title representation) -->
                            <div style="flex: 1; width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 4px;">
                                <div style="color: #fff; font-size: 5px; font-weight: 700; text-align: center; text-shadow: 0 1px 2px rgba(0,0,0,0.5); font-family: sans-serif; line-height: 1.1; width: 90%; word-wrap: break-word;">${titleText.substring(0, 15)}...</div>
                                <div style="width: 24px; height: 24px; background: rgba(255,255,255,0.2); border-radius: 4px; margin-top: 2px;"></div>
                                <div style="width: 32px; height: 3px; background: rgba(255,255,255,0.4); border-radius: 2px;"></div>
                                <div style="width: 20px; height: 3px; background: rgba(255,255,255,0.2); border-radius: 2px;"></div>
                            </div>

                            <!-- Screen Glare -->
                            <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(135deg, rgba(255,255,255,0.3) 0%, transparent 40%); border-radius: 6px; pointer-events: none;"></div>
                        </div>
                    </div>
                    <div style="padding: 16px; flex: 1; display: flex; flex-direction: column; justify-content: space-between; transform: translateZ(10px);">
                        <div>
                            <h4 style="font-size: 14px; font-weight: 700; margin: 0 0 6px; color: var(--text-primary);">${tpl.title}</h4>
                            <p style="font-size: 11.5px; color: var(--text-secondary); margin: 0 0 16px; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden;">${tpl.description}</p>
                        </div>
                        <button class="modal-btn load-template-action-btn" data-template-id="${tpl.id}" style="background: var(--accent); color: #fff; width: 100%; padding: 10px; border: none; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: opacity 0.2s; box-shadow: 0 2px 8px rgba(10, 132, 255, 0.3);">Load Template</button>
                    </div>
                </div>
            </div>
            `;
        ...
        // 3D Card Hover Effect (AuraMock 3D dynamic tilt)
        gridContainer.querySelectorAll('.template-card-wrapper').forEach(wrapper => {
            const card = wrapper.querySelector('.template-card');
            ...
            wrapper.addEventListener('mousemove', e => {
                ...
                card.style.boxShadow = `
                    ${-rotateY}px ${rotateX + 10}px 20px rgba(0,0,0,0.2), 
        ...
        gridContainer.querySelectorAll('.load-template-action-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const tplId = btn.dataset.templateId;
                loadExampleTemplate(tplId);
                modal.classList.remove('visible');
            });
        });
    }
...
function loadExampleTemplate(templateId) {
    const tpl = exampleTemplates.find(t => t.id === templateId);
    if (!tpl) return;

    // Create a new project ID
    const newProjectId = 'template-' + templateId + '-' + Date.now();
    const newProjectName = 'Template: ' + tpl.title.split(' (')[0];

    // Build the default background and screenshot structures
    const background = {
    ...
    // Construct elements
    const elements = [];
    if (tpl.badges && tpl.badges.length > 0) {
        tpl.badges.forEach((badgeType, i) => {
            const src = generateBadgeDataURL(badgeType);
            
            // App Store / Google Play positions
            let x = 50;
            let y = 15;
            let width = 20;

            if (badgeType === 'appstore') { x = 32; y = 18; width = 22; }
            else if (badgeType === 'googleplay') { x = 68; y = 18; width = 22; }
            else if (badgeType === 'stars') { x = 50; y = 88; width = 25; }
            else if (badgeType === 'laurel-appday') { x = 50; y = 85; width = 25; }
    ...
    const screenItem = {
        image: null, // User uploads their own screenshot inside the frame
        name: 'Mockup Screen',
        deviceType: state.outputDevice,
        localizedImages: {},
    ...
    // Add to project list
    const newProject = { id: newProjectId, name: newProjectName, screenshotCount: 1 };
    projects.push(newProject);
    saveProjectsMeta();

    // Switch project
    currentProjectId = newProjectId;
    state.screenshots = [screenItem];
    state.selectedIndex = 0;
    ...
    // Force set rotation in ThreeJS
    if (screenshot.use3D && typeof setThreeJSRotation === 'function') {
        setThreeJSRotation(screenshot.rotation3D.x, screenshot.rotation3D.y, screenshot.rotation3D.z);
    }
